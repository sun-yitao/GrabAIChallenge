...
class BAP(nn.Module):
    ...
    def forward(self, features, attentions):
        B = features.size(0) # shape ()
        M = attentions.size(1) # shape ()
        for i in range(M):
...
class WSDAN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.baseline == 'efficientnet':
            feature_maps = self.net.extract_features(x) # torch.Size([batchsize, 384, 16, 16])
        else:
        ...
        if self.training:
            ...
            attention_map = torch.zeros(batch_size, 1, H, W).to(torch.device(device))  # (B, 1, H, W)
